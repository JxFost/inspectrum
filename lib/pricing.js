...
export function calculatePrice({
  ...
  const base = basePriceBySqft(sqftNum)
  breakdown.push({ label: `Base (${sqftNum ? sqftNum + ' sqft' : 'est.'})`, amount: base })

  // Age surcharge
  const age = ageSurcharge(yearBuilt)
  if (age > 0) {
    const propertyAge = new Date().getFullYear() - parseInt(yearBuilt, 10)
    breakdown.push({ label: `Age surcharge (${propertyAge} yrs)`, amount: age })
  }
  ...
  if (cityCharge === null) {
    cityUnknown = true // unknown city — may need manual trip charge
  } else if (cityCharge > 0) {
    breakdown.push({ label: `Location (${city})`, amount: cityCharge })
  }

  // Feature add-ons
  if (features.additionalKitchen) breakdown.push({ label: 'Additional kitchen', amount: FEATURE_PRICES.additionalKitchen })
  if (features.additionalFurnaceCloset) breakdown.push({ label: 'Additional furnace (closet/basement)', amount: FEATURE_PRICES.additionalFurnaceCloset })
  if (features.additionalFurnaceAttic) breakdown.push({ label: 'Additional furnace (attic/crawl)', amount: FEATURE_PRICES.additionalFurnaceAttic })
  if (features.detachedGarage) breakdown.push({ label: 'Detached garage', amount: FEATURE_PRICES.detachedGarage })
  if (features.pool) breakdown.push({ label: 'Pool', amount: FEATURE_PRICES.pool })
  if (features.lawnSprinklers) breakdown.push({ label: 'Lawn sprinklers', amount: FEATURE_PRICES.lawnSprinklers })
  if (features.outbuildingFull) breakdown.push({ label: 'Outbuilding (full)', amount: FEATURE_PRICES.outbuildingFull })
  if (features.outbuildingElecOnly) breakdown.push({ label: 'Outbuilding (elec only)', amount: FEATURE_PRICES.outbuildingElecOnly })
  if (features.outbuildingStructure) breakdown.push({ label: 'Outbuilding (structure)', amount: FEATURE_PRICES.outbuildingStructure })
