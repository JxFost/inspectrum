...
// Prices include standard kitchen ($25) — these are the all-in base rates
const SQFT_TIERS = [
  { min: 1, max: 1000, price: 430 },
  { min: 1001, max: 1999, price: 455 },
  { min: 2000, max: 2499, price: 480 },
  { min: 2500, max: 2999, price: 505 },
  { min: 3000, max: 3499, price: 555 },
  { min: 3500, max: 3999, price: 580 },
  { min: 4000, max: 4499, price: 605 },
]

function basePriceBySqft(sqft) {
  if (!sqft || sqft <= 0) return 480 // default to mid-range
  for (const tier of SQFT_TIERS) {
    if (sqft >= tier.min && sqft <= tier.max) return tier.price
  }
  // 4500+ : $655 + $50 per additional 500 sqft
  if (sqft >= 4500) {
...
export function calculatePrice({
  ...
  // Kitchen is already included in base tiers above
