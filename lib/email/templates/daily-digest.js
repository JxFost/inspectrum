 ...
 */
export function dailyDigestHtml({ dateLabel, inspections, unsignedAgreements = [], missingReports = [] }) {
...
${unsignedAgreements.length > 0 ? `
          <div style="margin-top:20px;background-color:#FEF3C7;border:1px solid #E89A3F;border-radius:6px;padding:16px;">
            <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.1em;color:#E89A3F;font-weight:600;margin-bottom:8px;">Unsigned Agreements (${unsignedAgreements.length})</div>
            ${unsignedAgreements.map((a) => `<div style="font-size:13px;color:#1F2426;padding:3px 0;">${a.customer_name} — ${a.inspection_number || ''} · ${new Date(a.start_at).toLocaleDateString('en-US', { timeZone: 'America/Denver', month: 'short', day: 'numeric' })}</div>`).join('')}
          </div>
` : ''}

${missingReports.length > 0 ? `
          <div style="margin-top:12px;background-color:#FAF7F1;border:1px solid #E2DDD5;border-radius:6px;padding:16px;">
            <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.1em;color:#9DA0A2;font-weight:600;margin-bottom:8px;">Reports Not Sent (${missingReports.length})</div>
            ${missingReports.map((r) => `<div style="font-size:13px;color:#3D3F40;padding:3px 0;">${r.customer_name} — ${r.inspection_number || ''} · ${new Date(r.start_at).toLocaleDateString('en-US', { timeZone: 'America/Denver', month: 'short', day: 'numeric' })}</div>`).join('')}
          </div>
` : ''}
