 ...
 */
export function followupHtml({ customerName, service, startISO, address, token }) {
            ...
            <tr><td>
              ...
              <table cellpadding="0" cellspacing="0"><tr>
                ${[1,2,3,4,5].map((n) => `<td style="padding:0 4px;">
                  <a href="${siteUrl}/api/feedback?rating=${n}&name=${encodeURIComponent(customerName)}&service=${encodeURIComponent(service)}${token ? '&token=' + token : ''}" style="text-decoration:none;font-size:32px;color:#E89A3F;" title="${n} star${n !== 1 ? 's' : ''}">★</a>
                </td>`).join('')}
              </tr></table>
