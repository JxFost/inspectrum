            ...
            <tr><td>
              ...
              <table cellpadding="0" cellspacing="0"><tr>
                ${[1,2,3,4,5].map((n) => `<td style="padding:0 4px;">
                  <a href="${siteUrl}/api/feedback?rating=${n}&name=${encodeURIComponent(customerName)}&service=${encodeURIComponent(service)}" style="text-decoration:none;font-size:32px;color:${n <= 3 ? '#D1D5DB' : '#E89A3F'};" title="${n} star${n !== 1 ? 's' : ''}">${n <= 3 ? '☆' : '★'}</a>
                </td>`).join('')}
              </tr></table>
