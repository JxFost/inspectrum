...
/**
 * Fetch inspections from the DB within a date range.
 * Returns rows shaped to match the calendar-based format used by the dashboard.
 */
export async function getInspectionsFromDB(fromISO, toISO) {
  const db = sql()
  const rows = await db`
    SELECT
      google_event_id,
...
/**
 * Count inspections in a date range (excludes cancelled).
 */
export async function countInspections(fromISO, toISO) {
  const db = sql()
  const rows = await db`
    ...
    WHERE start_at >= ${fromISO}
      AND start_at <= ${toISO}
      AND status != 'cancelled'
      AND inspection_number IS NOT NULL
  `
...
/**
 * Move an inspection to a new start/end time by its booking token (reschedule).
 */
export async function rescheduleByToken(token, startISO, endISO) {
  const db = sql()
  const rows = await db`
    UPDATE inspections
    SET start_at = ${startISO}, end_at = ${endISO}, updated_at = now()
    WHERE token = ${token}
    RETURNING *
  `
  return rows[0] || null
