 ...
 *   GOOGLE_CALENDAR_ID           — the primary calendar to query / write to
 *   GOOGLE_BUSY_CALENDAR_IDS     — comma-separated additional calendar IDs to check for conflicts
 */
...
export async function getBusyRanges(timeMin, timeMax) {
  ...
  // Build list of all calendars to check for conflicts
  const busyExtras = process.env.GOOGLE_BUSY_CALENDAR_IDS
    ? process.env.GOOGLE_BUSY_CALENDAR_IDS.split(',').map((id) => id.trim())
    : []
  const allCalendarIds = [calendarId, ...busyExtras]
  ...
  // Merge busy ranges from all calendars
  const allBusy = allCalendarIds.flatMap(
    (id) => res.data.calendars[id]?.busy || [],
  )

  // Sort by start time so downstream slot logic works correctly
...
/**
 * Patch arbitrary event fields (summary, description, location, start/end).
 */
...
/**
 * Move an event to a new start/end time (for reschedules).
 */
export async function updateEventTime(eventId, startISO, endISO) {
  const auth = getAuth()
  await auth.authorize()
  const calendar = google.calendar({ version: 'v3', auth })
  const calendarId = process.env.GOOGLE_CALENDAR_ID
